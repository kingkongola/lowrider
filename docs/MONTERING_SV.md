...
5. **X-brytarens kabel, bilder `co.jpg`–`cq.jpg`:** Kapa **250 cm** från din **10 m Tensility 3×26 AWG-rulle** och märk biten `X` i båda ändar. Det är vår **tilltagna arbetslängd för 650×1250-maskinen**, inte ett mått från V1E. För ena änden genom Cores kabeltunnel (`co.jpg`), anslut brytaren till **COM + NC** med två av de tre ledarna enligt aktuell Jackpot3-koppling och montera den med M2,5-skruvarna enligt `cp.jpg`–`cq.jpg`; isolera den oanvända ledaren. Låt **resten av de 250 cm sitta kvar** för den rörliga vägen via mitten av balken till Jackpot3 på YZ_Min. **Kapa inte av överlängden vid Core.** Om den redan levererade brytaren har ansluten kabel, kontrollera kontakt/skarv i stället för att klippa bort fungerande ledning. Bilder `ck.jpg`–`cm.jpg` gäller främst en **valfri touchplate**; dess minst **140 mm fria kabel** är inte X-brytarens kapmått. Efter monterad balk: prova hela X-/Z-rörelsen innan eventuell ytterligare nedkortning.
