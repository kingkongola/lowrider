...
5. **X-brytarens kabel, bilder `co.jpg`–`cq.jpg` – KORRIGERAD PASSNING:** Användaren har faktiskt provat den köpta **Tensility `30-00377` (svart mantel, cirka Ø4 mm, röd/vit/svart 26 AWG)** och den **går inte genom Cores tunnel med manteln kvar**. **Försök inte tvinga in den och kapa inte hela kabeln till en gissad längd.** Två praktiska alternativ: **A)** använd två individuellt isolerade innerledare endast genom den trånga, fasta Core-tunneln. För detta, skala varsamt bort *enbart så mycket ytterhölje som behövs för själva tunneln och anslutningen*, utan att skada ledarisoleringen; låt resten av kabeln behålla manteln. Prova först kort sträcka och kontrollera att ledarna löper utan att nypas; avlasta manteln direkt utanför Core och skydda övergången mot skav (exempelvis lämplig krympslang eller skyddshylsa där den passar). **B)** om även de isolerade ledarna inte kan dras skyddat: använd en tunnare flexibel tvåledarkabel eller en säker alternativ yttre kabelväg, inte en oskyddad ledarslinga. Märk `X`. Anslut brytaren via **COM + NC** enligt dess märkning och kontrollera kontaktdonen innan krimpning; isolera oanvänd tredje ledare vid ändarna. X-brytaren monteras med **M2,5-skruvar**, brytararmen nedåt enligt `cp.jpg`–`cq.jpg`. **Ingen slutkapning nu**: kabeln ska senare löpa avlastat via balkens mitt till Jackpot3 på YZ_Min och kontrolleras genom full X-rörelse. Om 250 cm redan hade kapats är biten inte bortkastad; måttet var en obekräftad uppskattning. Bilder `ck.jpg`–`cm.jpg` gäller valfri *touchplate*; V1E:s minst **140 mm fri ledning** avser den, inte X-brytaren.
